...
Serves the terminal UI from /ui/index.html.
"""
...
async def get_recommendations(n: int = 6):
    """Top N trade recommendations with full Crypto AI analysis."""
    n = min(max(1, n), 12)
...
async def boot_agent():
    STATE.booted = True
    STATE.add_alert("BOOT", "Crypto Agent initialised -- FinBERT loaded, correlations computed", "INFO")
    await WS_MANAGER.broadcast({"type": "AGENT_BOOT", "message": "CRYPTO AGENT ONLINE"})
