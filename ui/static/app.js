...
function applyHealth(d) {
  ...
  setEl('divStatus',    d.portfolio_diversified ? '✓ DIVERSIFIED' : '✗ CONCENTRATED');
...
function signalCardHTML(s) {
  ...
  const overview  = s.ultra_justification?.ai_overview ?? '';
...
(function initMatrixBg() {
  ...
  function resize() {
    ...
    drops = new Array(columns).fill(0).map(() => Math.random() * -100 | 0);
  }
  resize();
  window.addEventListener('resize', resize);

  function draw() {
    // Subtle fade — low alpha for slow trail decay
    ctx.fillStyle = 'rgba(12, 10, 9, 0.06)';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    ctx.font = fontSize + 'px monospace';
    for (let i = 0; i < columns; i++) {
      // Only draw ~15% of columns per frame for sparsity
      if (Math.random() > 0.15) continue;

      const y = drops[i] * fontSize;
      // Mix of orange (primary) and green (terminal) characters
      if (Math.random() > 0.7) {
        ctx.fillStyle = 'rgba(245, 158, 11, 0.5)';  // orange accent
      } else {
        ctx.fillStyle = 'rgba(34, 197, 94, 0.35)';   // green terminal
      }
  ...
  // Delay start slightly so it doesn't compete with page load
